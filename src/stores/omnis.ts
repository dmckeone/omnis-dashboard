...
declare global {
  interface Window {
    ctrl_omnis_dashboard_hooks: any
  }
}
...
export const useOmnis = defineStore("Omnis", () => {
  ...
  */
  ...
  if (typeof window.ctrl_omnis_dashboard_hooks == "undefined") {
    window.ctrl_omnis_dashboard_hooks = new Map()
  }
  ...
  window.ctrl_omnis_dashboard_hooks.set(control_id, hooks)

  function emit(omnisEvent: { event: string; payload: string }) {
    const hooks = window.ctrl_omnis_dashboard_hooks.get(control_id)
